...
export const isGif = (type) =>
  isPreviewableImage && type.startsWith("image/gif");
...
export const isMarkdown = (filename = "", type = "") => {
  return (
    filename.toLowerCase().endsWith(".md") || type.startsWith("text/plain")
  );
};
...
const isLinkWithSource = (source) => (file) =>
  file.isLink && file.source === source;
